...
public class XCRoundProgressBar extends View {
    ...
    public XCRoundProgressBar(Context context, AttributeSet attrs,
        ...
        innerRoundColor = typedArray.getColor(R.styleable.XCRoundProgressBar_innerRoundColor, Color.WHITE);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float textWidth = paint.measureText(percent + "%"+ "  正确率");//测量字体宽度，需要居中显示

        if (isDisplayText && style == STROKE) {
            if (percent != 0) {
                canvas.drawText(percent + "%"+ "  正确率", centerX - textWidth / 2, centerX + textSize / 2, paint);
            }else{
                canvas.drawText(0 + "%" + "  正确率", centerX - textWidth / 2, centerX + textSize / 2, paint);
            }
        }
